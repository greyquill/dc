...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    /* Substack accepts no share URL. action=compose does open the Notes
       composer, but the text parameter beside it is ignored, so the box arrives
       empty. Copy the title and link on the way through and the paste is ready.
       Without action=compose this lands on the Notes feed instead, with no
       composer at all. The href is set here as well as in the markup: the two
       files are cached independently, so a page held in cache must not end up
       with a dead button. */
